...
public function beforeFilter() {
    parent::beforeFilter();   
    $this->Auth->allow('fetchUnions');
    $this->Auth->allow('fetchUnionsWithAccounts');
 ...
 */
	public function overview($id = null) {
		...
		if($uType == 8){
			if($ref_id != $id){
				$this->Session->setFlash(
	                __('You are not authorized to access another Unions Details other than yours..'),
	                'default',
	                array(),
	                'auth'
	            );
	            return $this->redirect(array('controller' => 'unions', 'action' => 'overview',$ref_id));
			}
		}
		if (!$this->Union->exists($id)) {
			throw new NotFoundException(__('Invalid union'));
		...
		if($uType == 7){ //Upazilla Level user
			if($ref_id != $unionDetail['Upazilla']['id']){
				$this->Session->setFlash(
 ...
 */
	public function add() {
		...
		if ($this->request->is('post')) {
			...
			if($this->Session->read('Auth.User.usertype_id')==7){ //for upazilla level users
				$this->request->data['Union']['upazilla_id'] = $ref_id;
			}
			$this->Union->create();
			if ($this->Union->save($this->request->data)) {
				$this->Session->setFlash(__('The union has been saved.'));
				return $this->redirect(array('action' => 'overview',$this->Union->id));
			} else {
 ...
 */
	public function edit($id = null) {
		...
		if ($this->request->is(array('post', 'put'))) {
			if ($this->Union->save($this->request->data)) {
				$this->Session->setFlash(__('The union has been updated.'));
