 ...
 */
	public function assign($id = null) {
		...
		$User_union_id = $this->Session->read('Auth.User.ref_id');
		$this->loadModel('Employee',$id);
		$this->Employee->recursive = -1;
		$empData = $this->Employee->read();
		$emp_union = $empData['Employee']['union_id'];
		if($UserType==8){ //condition for union level user only
			if($emp_union != $User_union_id){
				$this->Session->setFlash(
	                __('You are not authorized to add attendances to Employees outside your Union.'),
	                'default',
	                array(),
	                'auth'
	            );
	            return $this->redirect(array('controller' => 'users', 'action' => 'dashboard',$this->Session->read('Auth.User.id')));
			}
		}
		if ($this->request->is('post')) {
