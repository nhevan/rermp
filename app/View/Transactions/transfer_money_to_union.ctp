...
<?
	...
	$this->Js->get('#UpazillaId');
	$this->Js->event(
	    'change',
	    $this->Js->request(
	        array('action' => 'fetchUnionsWithAccounts', 'controller'=>'unions'),
	        array(
	        'async' => true, 
	        'update' => '#UnionAccountId',
	        'dataExpression' => true, 
            'method' => 'post', 
            'data' => $this->Js->serializeForm(array('isForm' => false, 'inline' => true))
            )
...
<?php echo $this->Form->create('Transaction'); ?>
	<fieldset>
		<legend><?php echo __('Transfer Money To Union Account'); ?></legend>
	...
	<div class="input select">
		<label for="UpazillaId">Choose Upazilla :</label>
		<select name="Upazilla" id="UpazillaId">
			<option>Choose a Upazilla</option>
		</select>
	</div>
	<div class="input select">
		<label for="UnionAccountId">Choose Union: :</label>
		<select name="data[Transaction][account_id]" id="UnionAccountId">
			<option>Choose a Union</option>
		</select>
	</div>
	<div id="test"></div>
	</fieldset>
