...
<h1><?php echo __($division['Division']['name'].' Division Overview'); ?>
<?php 
$uType = $this->Session->read('Auth.User.usertype_id');
if($uType==2){
echo $this->Html->link(__('Edit Division'), array('controller' => 'Divisions', 'action' => 'edit',$division['Division']['id'] ),array('class'=>'btn btn-danger pull-right')); 
}
?>
</h1>
...
<div>
	<h3><?php echo __('District List'); ?>
	<?php 
	$uType = $this->Session->read('Auth.User.usertype_id');
if($uType==2){
	echo $this->Html->link(__('Add New District'), array('controller'=>'Districts','action' => 'add'),array('class'=>'btn btn-warning')); 
	}
	?>
	</h3>
	<table class="table table-bordered" style="width:auto">
		...
		<?
			...
			//	foreach($district['Upazilla'] as $upazilla){
				?>
				<tr>
					<td><? echo $disCount++;//echo $district['id']; ?></td>
					...
					<td><?php echo $this->Html->link(__('District Detail'), array('controller' => 'districts', 'action' => 'overview',$district['id'] ),array('class'=>'btn btn-info')); ?> 
					<?
						if($usertype == 2){ //if HQ level user
							echo $this->Html->link(__('Edit'), array('controller' => 'Districts', 'action' => 'edit',$district['id'] ),array('class'=>'btn btn-info'));
						}
						if($usertype == 5){ //if Division level user
							//echo "<td>";
							//echo $this->Html->link(__('Send Fund'), array('controller' => 'transactions', 'action' => 'DivUserFundDistrict',$district['id'] ),array('class'=>'btn btn-info'));
							//echo "</td>";
						}
					?>	
